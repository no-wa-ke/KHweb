//TUMBLR json photosets/photo embed on html
//builder @kido_yoji 2014


/* important vales for CSS
tumblr body　<div class ="tumblr-wrap">
 post wrapper 　<div class ="postwrap">
 captions <div class="p-caption">
 link to photos <a class ="block-img">
            */
            ...
            switch (p.type) {
                case "photo":
                    if (p.photos.length < 1) { // photo: 一つなら
                        ...
                        for (d = 0; d < ppp; d++) {
                            var getWrap = "<div id ='mulWrap" + d + "'>";
                        }

                        toAppend = toAppend + getWrap + mulAppend + "</div>";
                        totalWrap = toWrap + toAppend + "</div>";
                        $("#tumblr-wrap").append(totalWrap);

                        // with jQuery
                        for (d = 0; d < ppp; d++) {
                            var container = '#mulWrap'+ d;
                        $(container).masonry({
                            columnWidth: 500,
                            itemSelector: '.mulpic'
                        });
